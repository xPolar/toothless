...
export default class GuildMemberAdd extends EventHandler {
	...
	public override async run({
		...
		this.client.dataDog.increment("guild_members", 1, [
			`guildId:${member.guild_id}`
		]);

		const newInvites = await this.client.api.guilds.getInvites(
			member.guild_id
		);
		...
		const usedInvite = newInvites.find(
			(invite) => invite.uses > (invitesCache?.get(invite.code) ?? 0)
		);

		if (usedInvite) {
			this.client.dataDog.increment("guild_joins", 1, [
				`guildId:${member.guild_id}`,
					...
					embeds: [
						{
							author: {
								name: "Member Joined",
								icon_url:
									"https://cdn.karalite.com/user_2Z1iHO0Px2py0XMFB2sy1mb631L/auditMemberCreate.png"
							},
							description: `**User:** <@${member.user!.id}> \`[${
								member.user!.id
							}]\`\n**Account Created:** ${this.client.functions.generateTimestamp(
								{
									timestamp: DiscordSnowflake.timestampFrom(
										member.user!.id
									)
								}
