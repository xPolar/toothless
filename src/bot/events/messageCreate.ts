...
export default class MessageCreate extends EventHandler {
	...
	public override async run({
			...
			if (error instanceof DiscordAPIError) {
				if (error.code === RESTJSONErrorCodes.UnknownChannel)
					this.client.logger.error(
						`Unable to fetch channel ${message.channel_id}.`
					);
			} else throw error;
		}
		...
		if (
			...
			const parentChannel = await this.client.api.channels.get(
				message.channel_id
			);
			...
			if (
				...
				(parentChannel.type === ChannelType.GuildForum &&
					parentChannel.parent_id !==
						this.client.config.supportCategoryId)
			)
				this.client.dataDog.increment(
					"total_messages_sent.engaging",
					1,
					[
						`guildId:${message.guild_id ?? "@me"}`,
						`userId:${message.author.id}`,
			...
			channel.parent_id !== this.client.config.supportCategoryId
		) {
		...
		if (message.guild_id) {
			if (
				new Date(message.member!.joined_at).getTime() >
