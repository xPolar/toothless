	...
	public override async run({
		...
		if (auditLogEntry.action_type === AuditLogEvent.ChannelCreate) {
			const loggingChannels =
				await this.client.prisma.logChannel.findMany({
					where: {
			...
			const channel = {
				name: auditLogEntry.changes!.find(
					(change) => change.key === "name"
				)!.new_value,
				type: auditLogEntry.changes!.find(
					(change) => change.key === "type"
				)!.new_value as ChannelType
			};
							...
							embeds: [
								{
									author: {
										...
										icon_url:
											"https://cdn.karalite.com/user_2Z1iHO0Px2py0XMFB2sy1mb631L/auditChannelCreate.png"
									},
									description: `**User:** ${
										auditLogEntry.user_id
											? `<@${auditLogEntry.user_id}> \`[${auditLogEntry.user_id}]\``
											: "Unknown User"
									}\n\n**Channel Name:** ${
										channel.name
									}\n**Channel Type:** ${
										this.channelTypesMap[channel.type]
									}`,
				...
				auditLogEntry.changes
					?.map((change) => {
						...
						if (change.key === "type")
							return {
								name: `**Channel Type:** ${
									this.channelTypesMap[
										change.old_value as ChannelType
									]
								}`,
								order: 2
							};
						...
						if (change.key === "nsfw")
							return change.old_value
								? { name: "**NSFW:** Yes", order: 3 }
								: null;

						if (change.key === "rate_limit_per_user")
							return change.old_value
								? {
										name: `**Slowmode:** ${this.client.functions.format(
											change.old_value * 1_000
										)}`,
										order: 4
								  }
								: null;
							...
							embeds: [
								{
									author: {
										...
										icon_url:
											"https://cdn.karalite.com/user_2Z1iHO0Px2py0XMFB2sy1mb631L/auditChannelDelete.png"
									},
									description: `**User:** ${
										auditLogEntry.user_id
											? `<@${auditLogEntry.user_id}> \`[${auditLogEntry.user_id}]\``
						...
						if (change.key === "type")
							return {
								name: `**Channel Type:** ${
									...
									this.channelTypesMap[
										change.new_value as ChannelType
									]
								}`,
								order: 2
							};
							...
							embeds: [
								{
									author: {
										...
										icon_url:
											"https://cdn.karalite.com/user_2Z1iHO0Px2py0XMFB2sy1mb631L/auditChannelUpdate.png"
									},
									description: `**User:** ${
										auditLogEntry.user_id
											? `<@${auditLogEntry.user_id}> \`[${auditLogEntry.user_id}]\``
							...
							embeds: [
								{
									author: {
										name: "Role Created",
										icon_url:
											"https://cdn.karalite.com/user_2Z1iHO0Px2py0XMFB2sy1mb631L/auditRoleCreate.png"
									},
									description: `**User:** ${
										auditLogEntry.user_id
											? `<@${auditLogEntry.user_id}> \`[${auditLogEntry.user_id}]\``
							...
							embeds: [
								{
									author: {
										name: "Role Deleted",
										icon_url:
											"https://cdn.karalite.com/user_2Z1iHO0Px2py0XMFB2sy1mb631L/auditRoleDelete.png"
									},
									description: `**User:** ${
										auditLogEntry.user_id
											? `<@${auditLogEntry.user_id}> \`[${auditLogEntry.user_id}]\``
							...
							embeds: [
								{
									author: {
										name: "Role Updated",
										icon_url:
											"https://cdn.karalite.com/user_2Z1iHO0Px2py0XMFB2sy1mb631L/auditRoleUpdate.png"
									},
									description: `**User:** ${
										auditLogEntry.user_id
											? `<@${auditLogEntry.user_id}> \`[${auditLogEntry.user_id}]\``
				...
				auditLogEntry.changes
					?.map((change) => {
						if (change.key === "$add" && change.new_value)
							return {
								name: `**Roles Added:** ${change.new_value
									.map((role) => `<@&${role.id}>`)
									.join(", ")}`,
								order: 1
							};
						...
						if (change.key === "$remove" && change.new_value)
							return {
								name: `**Roles Removed:** ${change.new_value
									.map((role) => `<@&${role.id}>`)
									.join(", ")}`,
								order: 2
							};
							...
							embeds: [
								{
									author: {
										name: "Member Updated",
										icon_url:
											"https://cdn.karalite.com/user_2Z1iHO0Px2py0XMFB2sy1mb631L/auditMemberUpdate.png"
									},
									description: `**User:** ${
										auditLogEntry.user_id
											? `<@${auditLogEntry.user_id}> \`[${auditLogEntry.user_id}]\``
			...
			const changes = (
				auditLogEntry.changes
					?.map((change) => {
						...
						if (
							change.key === "auto_archive_duration" &&
							change.new_value
						)
							return {
						...
						if (
							...
							return {
								name: `**Thread Slowmode:** ${this.client.functions.format(
									change.new_value * 1_000
								)}`,
								order: 4
							};
							...
							embeds: [
								{
									author: {
										name: "Thread Created",
										icon_url:
											"https://cdn.karalite.com/user_2Z1iHO0Px2py0XMFB2sy1mb631L/auditThreadCreate.png"
									},
									description: `**User:** ${
										auditLogEntry.user_id
											? `<@${auditLogEntry.user_id}> \`[${auditLogEntry.user_id}]\``
							...
							embeds: [
								{
									author: {
										name: "Thread Updated",
										icon_url:
											"https://cdn.karalite.com/user_2Z1iHO0Px2py0XMFB2sy1mb631L/auditThreadUpdate.png"
									},
									description: `**User:** ${
										auditLogEntry.user_id
											? `<@${auditLogEntry.user_id}> \`[${auditLogEntry.user_id}]\``
							...
							embeds: [
								{
									author: {
										name: "Thread Deleted",
										icon_url:
											"https://cdn.karalite.com/user_2Z1iHO0Px2py0XMFB2sy1mb631L/auditThreadDelete.png"
									},
									description: `**User:** ${
										auditLogEntry.user_id
											? `<@${auditLogEntry.user_id}> \`[${auditLogEntry.user_id}]\``
		...
		if (auditLogEntry.action_type === AuditLogEvent.InviteCreate) {
			const guildInvitesCache =
				this.client.invitesCache.get(auditLogEntry.guild_id) ??
				new Map();
			guildInvitesCache.set(
				auditLogEntry.changes?.find((change) => change.key === "code"),
				0
			);
			this.client.invitesCache.set(
				auditLogEntry.guild_id,
				guildInvitesCache
			);
							...
							embeds: [
								{
									author: {
										name: "Invite Created",
										icon_url:
											"https://cdn.karalite.com/user_2Z1iHO0Px2py0XMFB2sy1mb631L/auditLinkCreate.png"
									},
									description: `**User:** ${
										auditLogEntry.user_id
											? `<@${auditLogEntry.user_id}> \`[${auditLogEntry.user_id}]\``
							...
							embeds: [
								{
									author: {
										name: "Invite Deleted",
										icon_url:
											"https://cdn.karalite.com/user_2Z1iHO0Px2py0XMFB2sy1mb631L/auditLinkDelete.png"
									},
									description: `**User:** ${
										auditLogEntry.user_id
											? `<@${auditLogEntry.user_id}> \`[${auditLogEntry.user_id}]\``
