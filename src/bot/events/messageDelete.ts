...
export default class MessageDelete extends EventHandler {
	...
	public override async run({
		...
		if (
			message.author?.bot ||
			(message.type && [MessageType.ThreadCreated].includes(message.type))
		)
			return;
		...
		if (!this.client.channelNameCache.has(message.channel_id)) {
			try {
				...
				this.client.channelNameCache.set(
					message.channel_id,
					channel.name
				);
				setTimeout(
					() =>
						this.client.channelNameCache.delete(message.channel_id),
					30_000
				);
			} catch (error) {
				if (
		...
		const channelName = this.client.channelNameCache.get(
			message.channel_id
		);
		...
		const embedToSend = {
			author: {
				...
				icon_url:
					"https://cdn.karalite.com/user_2Z1iHO0Px2py0XMFB2sy1mb631L/auditMessageDelete.png"
			},
			description: `**User:** ${
				authorId ? `<@${authorId}> \`[${authorId}]\`` : "Unknown User"
			}\n**Channel:** <#${message.channel_id}> \`(#${channelName})\` \`[${
				message.channel_id
			}]\`\n${
